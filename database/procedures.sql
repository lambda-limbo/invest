...
--
DELIMITER //
CREATE  PROCEDURE P_INSERT_STOCK(STOCK_MINIMIUM DOUBLE, STOCK_MAXIMUN DOUBLE, STOCK_OPENING DOUBLE, STOCK_CLOSE_VALUE DOUBLE, 
                                    TB_COMPANY_COMPANY_PK INT, STOCK_VALUE DOUBLE)
...
BEGIN
  SELECT USER_LOGIN, USER_WALLET, USER_PK FROM TB_USER WHERE USER_LOGIN = LOGIN AND USER_PASSWORD = PASSWORD;
END
//
...
BEGIN
  ...
  FROM
    TB_COMPANY C, TB_TRANSACTION T, TB_STOCK
  WHERE 
    (TB_STOCK.STOCK_PK = T.STOCK_PK)
    AND (C.COMPANY_PK = TB_STOCK.TB_COMPANY_COMPANY_PK)
    AND T.USER_USER_PK_CODE = CODIGO;
END
//
...
--
DELIMITER //
CREATE  PROCEDURE P_INSERT_COMPANY (IN NOME VARCHAR(100), IN INFO VARCHAR(100), IN SYMBOL CHAR(5))
BEGIN
